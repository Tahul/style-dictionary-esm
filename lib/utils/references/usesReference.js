 ...
 */
export default function usesReference(value, referencesOptionsOrRegex = {}) {
  // `this` is the dictionary object passed to formats and actions
  const regex =
    referencesOptionsOrRegex instanceof RegExp
  ...
  if (typeof value === 'object') {
    ...
    for (const key in value) {
      if (value.hasOwnProperty(key)) {
        ...
        let reference = usesReference(element, referencesOptionsOrRegex);
        if (reference) {
          hasReference = true;
          break;
        }
